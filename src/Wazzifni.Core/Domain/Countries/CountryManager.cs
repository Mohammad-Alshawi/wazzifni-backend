    ...
    public class CountryManager : DomainService, ICountryManager
    {
        ...
        public async Task<Country> GetEntityByIdAsync(int id)
        {
            var entity = await _countryRepository.GetAll()
                ...
                .Include(c => c.Cities).ThenInclude(c => c.Regions)
        ...
        public async Task<List<string>> GetAllCountryNameForAutoComplete(string keyword)
        {
            return await _countryTranslationRepository.GetAll().Where(x => x.Name.Contains(keyword)).Select(x => x.Name).ToListAsync();
        }
